...
def send_reset_code(year, code, login, email):
    ...
    if email_smtp_server is None:
        return False, "Nenastaven email-smtp-server"
    if email_smtp_port is None:
        return False, "Nenastaven email-smtp-port"
    if email_smtp_auth is None:
        return False, "Nenastaven email-smtp-auth"
    if email_smtp_user is None:
        return False, "Nenastaven email-smtp-user"
    if email_smtp_password is None:
        return False, "Nenastaven email-smtp-password"
    if email_smtp_from is None:
        return False, "Nenastaven email-smtp-from"
    if base_url is None:
        return False, "Nenastaven base-url"
    ...
    message += "<p><p>V případě problémů nás kontaktujte emailem."
    message += "<p><p>Orgové"
    message += "</BODY></HTML>"
